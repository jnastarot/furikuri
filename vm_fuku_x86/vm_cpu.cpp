...
void WINAPI fuku_vm_handler(uint32_t original_stack) {
    ...
    context.real_context.regs.esp += 4 + 4; //eflags + ptr vm_code


    uint8_t __shell_pure[] = {
        0x89, 0x25, 0, 0, 0, 0,             //mov [vm_context_imm],esp
        0xBC, 0, 0, 0, 0,                   //load original context pointer
        0x61,                               //popad
        0x9D,                               //popfd
        0x8B, 0x25, 0, 0, 0, 0,             //mov esp, original esp
    ...
    memcpy(context.shell_pure, __shell_pure, sizeof(__shell_pure));
        ...
        */
        ...
        case vm_opcode_86_pure: {
            ...
            if (instruction->info.reloc_offset_1) {
                //*(uint32_t*)&inst_[instruction->info.reloc_offset_1] += vm_context.image_base - vm_context.original_image_base;
            }

            if (instruction->info.reloc_offset_2) {
                // *(uint32_t*)&shell_pure[instruction->info.reloc_offset_2] += vm_context.image_base - vm_context.original_image_base;
            }
        ...
        case vm_opcode_86_mul: {
          //  vm_mul(context);
            break;
        }
        case vm_opcode_86_imul: {
         //   vm_imul(context);
            break;
        }
        case vm_opcode_86_div: {
            vm_div(context);
            break;
        }
        case vm_opcode_86_idiv: {
            vm_idiv(context);
            break;
        }
...
void vm_pure(vm_context& context,uint8_t * instruction_ptr, uint32_t instruction_len) {
    ...
    */
    ...
    uint8_t * shell_ptr = context.shell_pure;

    for (int i = 0; i < 16; i++) {
        if (i < instruction_len) {
            shell_ptr[19 + i] = instruction_ptr[i];
        }
        else {
            shell_ptr[19 + i] = 0x90;
        }
    ...
    *(uint32_t*)&shell_ptr[2]  = (uint32_t)&shell_ptr[49];               //vm_context_imm
    *(uint32_t*)&shell_ptr[7]  = (uint32_t)&context.pure_call_context.saved_regs[0];
    *(uint32_t*)&shell_ptr[15] = (uint32_t)&context.pure_call_context.saved_regs[3];
    *(uint32_t*)&shell_ptr[37] = (uint32_t)&context.pure_call_context.saved_stack_pointer; //vm_context_imm
    *(uint32_t*)&shell_ptr[42] = ((uint32_t)&context.pure_call_context.saved_flags + 4);

	__asm {
		//save vm context
		pushad

		call dword ptr shell_ptr //call shellcode
