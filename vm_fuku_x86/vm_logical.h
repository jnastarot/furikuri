...
void vm_rcl(vm_context& context) {
    ...
    if (op_2) {
        uint64_t result = op_1 << op_2 | context.real_context.flags._cf << (op_2 - 1);
...
void vm_rcr(vm_context& context) {
    ...
    if (op_2) {
        uint64_t result = op_1 >> op_2 | context.real_context.flags._cf << (SIZE_TO_BITS(ex_code->info.op_1_size) - op_2);
