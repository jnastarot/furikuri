...
public:
    ...
    virtual std::vector<uint8_t> fuku_virtualizer::create_vm_jumpout(uint64_t src_address, uint64_t dst_address, uint64_t vm_entry_address, std::vector<fuku_code_relocation>& relocation_table) const = 0;
    virtual std::vector<uint8_t> fuku_virtualizer::get_bytecode() const = 0;

    virtual fuku_arch fuku_virtualizer::get_target_arch() const { return fuku_arch::fuku_arch_unknown; };
};
