...
bool _dec_64_reg_tmpl(mutation_context& ctx) {
...
bool _dec_64_op_tmpl(mutation_context& ctx) {
...
bool fukutate_64_dec(mutation_context& ctx) {

    auto detail = ctx.instruction->detail->x86;

    if (detail.operands[0].type == X86_OP_REG) {  //dec reg
        return _dec_64_reg_tmpl(ctx);
    }
    else if (detail.operands[0].type == X86_OP_MEM) { //dec [op]
        return _dec_64_op_tmpl(ctx);
    }
