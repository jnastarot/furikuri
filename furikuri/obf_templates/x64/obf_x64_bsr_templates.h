...
bool _bsr_64_reg_reg_tmpl(mutation_context& ctx) {
...
bool _bsr_64_reg_op_tmpl(mutation_context& ctx) {
...
bool fukutate_64_bsr(mutation_context& ctx) {
    ...
    if (detail.operands[0].type == X86_OP_REG) {
        ...
        if (detail.operands[1].type == X86_OP_REG) { //bsr reg, reg
            return _bsr_64_reg_reg_tmpl(ctx);
        }
        else if (detail.operands[1].type == X86_OP_MEM) {//bsr reg, [op]
            return _bsr_64_reg_op_tmpl(ctx);
        }
    }
